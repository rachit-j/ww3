...
<script>
    function sendSortRequest(sortType) {
        ...
        var queryString = 'input=' + encodeURIComponent(data);

        fetch('https://ww3.stu.nighthawkcodingsociety.com/api/sorting/' + sortType + '?' + queryString)
    ...
    function analyzeSorts() {
        ...
        fetch('https://ww3.stu.nighthawkcodingsociety.com/api/sorting/analyze?' + queryString)
        .then(response => response.json())
        .then(data => {
